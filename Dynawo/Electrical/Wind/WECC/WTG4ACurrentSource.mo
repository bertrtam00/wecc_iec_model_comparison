...
model WTG4ACurrentSource "WECC Wind Turbine model with a simplified drive train model (dual-mass model) and with a current source as interface with the grid"
  ...
  Modelica.Blocks.Sources.RealExpression OmegaRef1(y = OmegaRef.y) annotation(
    Placement(visible = true, transformation(origin = {14, -47}, extent = {{-6.5, -5.5}, {6.5, 5.5}}, rotation = 0)));
  ...
  Dynawo.Electrical.Controls.WECC.DriveTrainPmConstant driveTrainPmConstrant(Dshaft = Dshaft, Hg = Hg, Ht = Ht, Kshaft = Kshaft, PInj0Pu = PInj0Pu, PePu(start = PInj0Pu)) annotation(
    Placement(visible = true, transformation(origin = {39.1733, -47.936}, extent = {{-7.38218, -5.33158}, {7.38218, 5.33158}}, rotation = 0)));
...
equation
  ...
  connect(driveTrainPmConstrant.omegaGPu, wecc_reec.omegaGPu) annotation(
    Line(points = {{43.5, -41}, {43.5, -40}, {-10, -40}, {-10, -29}}, color = {0, 0, 127}));
  connect(OmegaRef1.y, driveTrainPmConstrant.omegaRefPu) annotation(
    Line(points = {{21, -47}, {32, -47}}, color = {0, 0, 127}));
